...
void msgCallback_vel(const geometry_msgs::Twist::ConstPtr& msg){
    if( msg->linear.x == 0.0 && msg->linear.y == 0.0 && msg->linear.z == 0.0 && msg->angular.x == 0.0 && msg->angular.y == 0.0 && msg->angular.z == 0.0 ) {
        ROS_INFO("STOPPED");
	stop_count++;
	if ( stop_count >3)
	    stop = true;
...
//    move_arm(0,0,0,0,move_group_arm);
    ...
    ros::Subscriber imu = nh.subscribe("/cmd_vel", 100, msgCallback_vel);
    while ( ros::ok() && !(stop) ) {
	usleep(100000);
        ros::spinOnce();
    }
